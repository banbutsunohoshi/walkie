...
Покрытие запускается автоматически через `pytest-cov` и выводится в консоль (порог 65%). При необходимости мы можем явно
указать параметры:

```bash
pytest --cov=cli --cov=domain --cov=use_cases --cov=infrastructure --cov=main --cov-report=term-missing --cov-fail-under=65
```
...
Проект организован по папкам и модулям следующим образом:

- `main.py` — точка входа, сборка зависимостей и запуск `WalkieApp`
- `cli/`
  - `menu.py` — класс `MainMenu` (главное меню, выбор действия)
  - `prompts.py` — класс `WalkPrompter` (опрос пользователя)
  - `views.py` — класс `WalkView` (вывод маршрута, заданий, итогов, истории)
- `use_cases/`
  - `generate_walk.py` — `GenerateWalkUseCase` (создание новой прогулки)
  - `finish_walk.py` — `FinishWalkUseCase` (завершение прогулки и сохранение результата)
  - `show_history.py` — `ShowHistoryUseCase` (работа с историей прогулок)
- `domain/`
...
- `infrastructure/`
  ...
  - `photo_storage.py` — локальное хранение фотографий в файловой системе
...
Структура хранения фото (с копированием в каталог данных):
  ...
  "tasks": [
    {
      ...
      "photos": [
        {
          "file_path": "photos/12/task_1/2025-11-25_1830_sunset.jpg",
...
- `infrastructure/photo_storage.py` — локальное хранилище фотографий
...
1. `main.py` → сборка `WalkieApp` → показ главного меню → пользователь выбирает «Новая прогулка»  
...
4. Вызывается `GenerateWalkUseCase`:  
...
7. При завершении или прерывании прогулки:
   - `FinishWalkUseCase` получает данные
...
2. `ShowHistoryUseCase` через `WalkStorage` читает `history.json`
